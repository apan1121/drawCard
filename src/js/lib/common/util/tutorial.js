...
function Tutorial(input_steps, config){
     ...
     * 執行 beforeAction 動作
     */
    const beforeCheckPromise = function(TmpIntroInfo, TmpTutorialNode){
        return new Promise((resolve, reject) => {
            /**
             * 如果 beforeAction 是 function
             */
            if (typeof TmpIntroInfo.beforeCheck === 'function') {
                TmpIntroInfo.beforeCheck(that, TmpIntroInfo, TmpTutorialNode, () => {
                    resolve();
                });
            } else {
                resolve();
    ...
    const beforeActionPromise = function(TmpIntroInfo, TmpTutorialNode){
        return new Promise((resolve, reject) => {
            ...
            if (typeof TmpIntroInfo.beforeAction === 'function') {
                TmpIntroInfo.beforeAction(that, TmpTutorialNode, () => {
                    resolve();
                });
            } else {
                resolve();
    ...
    this.run = function(keyword){
        ...
        runTimer = setTimeout(() => {
            ...
            if (!!IntroInfo && 1) {
                ...
                console.log('beforeCheck', typeof beforeCheck);
                 ...
                 */
                const RunAllPromise = async function(){
                    await beforeCheckPromise(IntroInfo, TutorialNode);
